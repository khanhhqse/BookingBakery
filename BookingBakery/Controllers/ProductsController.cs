...
namespace BookingBakery.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(ProductDto), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> UpdateStorageInstructions(
            int id, [FromBody] UpdateProductStorageInstructionsDto dto)
        {
            var product = await _productService.UpdateStorageInstructionsAsync(id, dto.StorageInstructions);
            if (product == null)
                return NotFound(new { message = $"Không tìm thấy sản phẩm với ID = {id}." });
